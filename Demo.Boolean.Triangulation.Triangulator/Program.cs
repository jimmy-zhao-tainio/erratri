...
namespace ConstrainedTriangulator.Demo
{
    internal static class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            bool legacy = Array.Exists(args, arg => arg == "--legacy");
            var result = legacy
                ? Triangulator.RunFast(in input, validate: true)
                : ConformingDelaunay.Run(in input);
            var path = Path.GetFullPath(legacy
                ? "constrained_triangulator_fast_legacy.png"
                : "constrained_triangulator_fast.png");
            Render(result.Points, result.Triangles, legacy ? constraints : result.Segments, path);
            Console.WriteLine(legacy ? "Legacy fast triangulation:" : "Conforming Delaunay triangulation:");
